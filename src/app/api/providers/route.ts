...
export async function GET(req: Request) {
    try {
        ...
        // If caller specifically requests providers, read from the `providers` table
        // as the canonical source for provider users, then resolve auth users and
        // attach profile rows.
        if (userType === 'Provider') {
            const { data: providers, error: providersErr } = await supabase
                .from('providers')
                .select('user_id,is_active,is_deleted,address,zip,state_id,city_id')
                .eq('is_deleted', false);
            ...
            for (const p of providers || []) {
                ...
                try {
                    ...
                    if (authUser) {
                        // expose provider metadata inside .provider and return user
                        users.push({
                            ...authUser,
                            profile,
                            provider: {
                    ...
                    console.warn('Failed to fetch auth user for provider', userId, e);
                }
            }
        ...
        // Default fallback: list auth users
...
export async function POST(req: Request) {
    try {
        ...
        const { email, password, name, role, userType, address, zip, state_id, city_id } = body;
        const supabase = getSupabaseAdmin();
        ...
        // Ensure a providers row exists when userType === 'Provider' (best-effort)
        try {
            ...
            if (userId && userType === 'Provider') {
                try {
                    ...
                    const upsertProvider: any = await supabase.from('providers').upsert([
                        upsertPayload
                    ], { onConflict: 'user_id' });
                    if (upsertProvider?.error) {
                        console.warn('Failed to upsert into providers table:', upsertProvider.error);
                    }
                } catch (pe) {
                    console.warn('Error upserting into providers table (likely table missing):', pe);
                }
            }
        } catch (ie) {
            console.warn('Failed to ensure providers row:', ie);
        }
...
export async function PATCH(req: Request) {
    try {
        const supabase = getSupabaseAdmin();
        const body = await req.json();
        const { action, patientId, isActive, name, email, role, password, address, zip, state_id, city_id } = body || {};
        ...
        if (action === 'toggle_active') {
            const { data, error } = await supabase.from('providers').update({ is_active: !!isActive }).eq('user_id', patientId);
            if (error) return NextResponse.json({ error: error.message || error }, { status: 400 });
            return NextResponse.json({ ok: true, data });
        }

        if (action === 'soft_delete') {
            const { data, error } = await supabase.from('providers').update({ is_deleted: true, is_active: false }).eq('user_id', patientId);
            if (error) return NextResponse.json({ error: error.message || error }, { status: 400 });
            return NextResponse.json({ ok: true, data });
        }
        ...
        if (action === 'edit') {
            try {
                ...
                // Persist address / location fields into providers row (best-effort)
                try {
                    ...
                    const upsertRes: any = await supabase.from('providers').upsert([payload], { onConflict: 'user_id' });
                    if (upsertRes?.error) console.warn('Failed to upsert into providers table:', upsertRes.error);
                } catch (pe) {
                    console.warn('Error upserting into providers table (likely table missing):', pe);
                }
