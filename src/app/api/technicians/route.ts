...
export async function GET(req: Request) {
    try {
        ...
        if (userType === 'Technician') {
            const { data: techniciansRows, error: techniciansErr } = await supabase
                .from('technicians')
                // include role_id and address/location fields so client can display and pre-select
                .select('user_id,is_active,is_deleted,role_id,address,zip,state_id,city_id')
                .eq('is_deleted', false);

            if (techniciansErr) throw techniciansErr;

            const users: any[] = [];
            for (const p of techniciansRows || []) {
                ...
                try {
                    ...
                    console.warn('Failed to fetch auth user for technician', userId, e);
                }
            }
        ...
        try {
            ...
            if (userId && userType === 'Technician') {
                try {
                    ...
                    const upsertRes: any = await supabase.from('technicians').upsert([
                        upsertPayload
                    ], { onConflict: 'user_id' });
                    if (upsertRes?.error) {
                        console.warn('Failed to upsert into technicians table:', upsertRes.error);
                    }
                } catch (pe) {
                    console.warn('Error upserting into technicians table (likely table missing):', pe);
                }
            }
        } catch (ie) {
            console.warn('Failed to ensure technicians row:', ie);
        }
...
export async function PATCH(req: Request) {
    try {
        ...
        if (action === 'toggle_active') {
            const { data, error } = await supabase.from('technicians').update({ is_active: !!isActive }).eq('user_id', patientId);
            if (error) return NextResponse.json({ error: error.message || error }, { status: 400 });
            return NextResponse.json({ ok: true, data });
        }

        if (action === 'soft_delete') {
            const { data, error } = await supabase.from('technicians').update({ is_deleted: true, is_active: false }).eq('user_id', patientId);
            if (error) return NextResponse.json({ error: error.message || error }, { status: 400 });
            return NextResponse.json({ ok: true, data });
        }
        ...
        if (action === 'edit') {
            try {
                ...
                try {
                    ...
                    const upsertRes: any = await supabase.from('technicians').upsert([payload], { onConflict: 'user_id' });
                    if (upsertRes?.error) console.warn('Failed to upsert into technicians table:', upsertRes.error);
                } catch (pe) {
                    console.warn('Error upserting into technicians table (likely table missing):', pe);
                }
